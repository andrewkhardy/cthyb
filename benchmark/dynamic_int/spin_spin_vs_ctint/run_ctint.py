...
from triqs.gfs import (MeshDLRImFreq, MeshDLRImTime, fit_gf_dlr, make_gf_dlr, make_gf_dlr_imfreq, make_gf_from_fourier,
                       make_gf_imtime)
...
S.solve(h_int=common.h_int(args.U), n_warmup_cycles=args.n_warmup_cycles, n_cycles=args.n_cycles,
        measure_M_iw=True, measure_M_tau=False, measure_chiAB_tau=True, chi_A_vec=[common.SZ], chi_B_vec=[common.SZ],
        post_process=True)


def first_component(g):
    return g.data.reshape(g.data.shape[0], -1)[:, 0].real


if mpi.is_master_node():
    # G_iw / chiAB_tau come on DLR meshes in some CTINT versions and on regular ones in others:
    # bring both to uniform tau grids.
    def g_tau_uniform(g_iw):
        if isinstance(g_iw.mesh, MeshDLRImFreq):
            return make_gf_imtime(make_gf_dlr(g_iw), common.N_TAU)
        return make_gf_from_fourier(g_iw, common.N_TAU)

    G_tau = {bl: g_tau_uniform(S.G_iw[bl]) for bl in ("up", "down")}
    ...
    if isinstance(chi_tau.mesh, MeshDLRImTime):
        # The DLR representation holds for the connected correlator only: take out <Sz>^2
        # before interpolating, add it back after (as in compare_ctint_ctseg.py).
    ...
    common.save_results(common.output_file(args, "ctint"), args, "ctint",
                        ...
                        common.tau_points(chi_tau), first_component(chi_tau) + disconnected,
